...
#    IPSCAR.R
#
#   Individual Phylogenetic Species Clustering -Area Relationship
#
#  $Version: 0.1 $  $Date: 2013/02/17 23:48:03 $
#
#   function to compute Indivdual Phylogenetic Species Clustering Area Relationships (i.e., acumulation of PSC curves
#    around individual species) in a community.
#
#    Value: an object of class fv , bassicaly a data.frame with the following
#    elemnts:
#     r: vector of radii at which IPSCAR(r) has been estimated
#     ipsv: vector with the IPSCAR(r) values
...
ipscar<- function(mippp, mippp.sp=NULL, mimark=NULL,  namesmark=NULL, tree=NULL,
  ...
  ipscar <- lapply(cosamt, function(x) psc(x, tree=tree)$PSCs)
  # then average ipsv for each radius
  ipscar <- sapply(ipscar, mean, na.rm=T) # TODO: optional na.rm. At short distances lot of NA's and maybe unreliable means

  # return results
  
   result <- data.frame(r=r, ipscar=ipscar)
  
   result <- fv(result, argu="r", ylab=substitute(IPSCAR(r), NULL),
              valu="ipscar", fmla=ipscar ~ r,
              alim=c(min(r),max(r)),
              labl=c("r", "%s(r)"),
