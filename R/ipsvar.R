...
#    IPSVAR.R
#
#   Individual Phylogenetic Species Variety -Area Relationship
#
#  $Version: 0.1 $  $Date: 2013/02/17 23:48:03 $
#
#   function to compute Indivdual Phylogenetic Species Variety Area Relationships (i.e., acumulation of PSV curves
...
#     ipsv: vector with the IPSVAR(r) values
...
#
#   Details:
#     bfw can be employed to select different habitats and compute ISAR((R) i 
#      each habitat
...
ipsvar<- function(mippp, mippp.sp=NULL, mimark=NULL,  namesmark=NULL, tree=NULL,
  ...
  #Compute IPSVAR(r) for each radius r
    # preset result to NA
    ipsvar<- rep(NA, length(r))
  
  # first for each individual (for those individuals which have enough neighbors)
     #ipsvar <- lapply(cosamt, function(x) psv(x, tree=tree, compute.var=F)$PSVs)
  ipsvar[cosamt.ok] <- lapply(cosamt[cosamt.ok], function(x) psv(x, tree = tree, compute.var = F)$PSVs)
  # then average ipsv for each radius
  ipsvar <- sapply(ipsvar, mean, na.rm=T) # TODO: optional na.rm. At short distances lot of NA's and maybe unreliable means

  # return results
  
   result <- data.frame(r=r, ipsvar=ipsvar)
  
   result <- fv(result, argu="r", ylab=substitute(IPSVAR(r), NULL),
              valu="ipsvar", fmla=ipsvar ~ r,
              alim=c(min(r),max(r)),
              labl=c("r", "%s(r)"),
