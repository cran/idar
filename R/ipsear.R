...
#    IPSEAR.R
#
#   Individual Phylogenetic Species Evennes -Area Relationship
#
#  $Version: 0.1 $  $Date: 2013/02/17 23:48:03 $
#
#   function to compute Indivdual Phylogenetic Species Evennes Area Relationships (i.e., acumulation of PSE curves
#    around individual species) in a community.
#
#    Value: an object of class fv , bassicaly a data.frame with the following
#    elemnts:
#     r: vector of radii at which IPSEAR(r) has been estimated
#     ipsv: vector with the IPSEAR(r) values
...
ipsear<- function(mippp, mippp.sp=NULL, mimark=NULL,  namesmark=NULL, tree=NULL,
  ...
  #Compute IPSVAR(r) for each radius r
   # preset result to NA
    ipsear<- rep(NA, length(r))
  
  # first for each individual  (for those individuals which have enough neighbors)
      #ipsear <- lapply(cosamt, function(x) pse(x, tree=tree)$PSE)
   ipsear[cosamt.ok] <- lapply(cosamt[cosamt.ok], function(x) pse(x, tree = tree)$PSE)
  
  
  # then average ipsv for each radius
  ipsear <- sapply(ipsear, mean, na.rm=T) # TODO: optional na.rm. At short distances lot of NA's and maybe unreliable means

  # return results
  
   result <- data.frame(r=r, ipsear=ipsear)
  
   result <- fv(result, argu="r", ylab=substitute(IPSEAR(r), NULL),
              valu="ipsear", fmla=ipsear ~ r,
              alim=c(min(r),max(r)),
              labl=c("r", "%s(r)"),
